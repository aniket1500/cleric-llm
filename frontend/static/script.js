...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function submitDocuments() {
        ...
        let documents = document.getElementById('documents').value.split(',').map(s => s.trim()).filter(Boolean);
    ...
    async function getFacts(taskId, resultSection) {
        try {
            ...
            if (data.status === 'done' && data.facts) {
                ...
                setTimeout(() => getFacts(taskId, resultSection), 1000);  // Re-poll after 1 second
            } else {
                resultSection.innerHTML = 'No facts available or processing not complete';
            }
        } catch (error) {
